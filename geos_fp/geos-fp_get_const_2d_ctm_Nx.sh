...
FALSE_DATE_TARGET_NAME=GEOS.fp.asm.const_2d_asm_Nx.${YMD}.nc4
...
if [ -s ${CONSTANTS_FILEPATH} ]; then
	cp ${CONSTANTS_FILEPATH} 2d_ctm/${FALSE_DATE_TARGET_NAME} 
	echo "${CONSTANTS_FILEPATH} already on disc, copying to 2d_ctm/${FALSE_DATE_TARGET_NAME}"
else
